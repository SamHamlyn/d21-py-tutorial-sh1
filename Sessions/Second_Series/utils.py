...
def format_dates(column):
    ...
    """
    column.replace(
        r"^\s*$", pd.NaT, regex=True
    )  # can use regex to find the empty strings ^\s*$ is the regex for blank space
...
def clean_903_table(df: pd.DataFrame, collection_end: pd.Timestamp) -> pd.DataFrame:
    ...
    clean_df = (
        df.copy()
    )  # make a copy that is worked on so not overwriitng orrginal data

    # remove index
    if "index" in df.columns:
        clean_df.drop(
            "index", axis=1, inplace=True
        )  # when axis = 0 it goes by row. when axis = 0 it drops rows

    # convert date columns
    for column in clean_df.columns:
        if column in DateCols.cols.value:
            clean_df[f"{column}_dt"] = format_dates(
                clean_df[column]
            )  # makes a new column with the suffix _dt
    ...
    if "DOB_dt" in clean_df.columns:
        clean_df["AGE"] = clean_df["DOB_dt"].apply(
            lambda dob: relativedelta(dt1=collection_end, dt2=dob)
            .normalized()
            .years  # dt1 is end date of collection. normalized takes it at midnight
        )
...
def group_calculation(df, column, measure_name):
    ...
    grouped[f"{measure_name} - Percentage"] = (
        grouped[f"{measure_name} - Count"] / grouped[f"{measure_name} - Count"].sum()
    ) * 100
